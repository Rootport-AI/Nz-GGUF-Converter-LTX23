...
その節が要求する検証を実際に行った結果の記録で、**実物の変換そのものはまだ行って
いません**（理由は8-1、残りの手順は8-5）。
...
- 想定される2種類のマーカーJSONを、Pythonの `json.dumps` の既定書式（`, ` と `: `）で
  ...
  2種類の長さは、この書式の違いで説明がつきます。ただし**本文そのものは欠落部に
  あって読めていないため、これは推測**です。確認は8-5-2で行います。

### 8-5. 未実施の作業（完全版を取り直したあとに行う）— 手順書

**以下はまだ実施していません。** 完全版のファイルを取得し直してから行う手順です。

#### 8-5-1. 実物の受け入れ検査と変換

```
run.bat inspect --model ltx25-comfyquant --st-path <完全版のsafetensors> ^
...
- marker variants 2（int8とw4a8の2種類。内容を正規化してから数えるため、8-4の書式の違いでは増えません）
- planned output の type_counts が `{BF16: 2401, F32: 290, Q6_K: 1658}`

続いて変換と、独立した自己検証を行います。

```
run.bat convert     --model ltx25-comfyquant --st-path <完全版のsafetensors>
run.bat self-verify --model ltx25-comfyquant --st-path <完全版のsafetensors>
```

合格条件: 変換が完走し、出力サイズがREADMEの表の値（Q6_Kなら19,631,143,808バイト）と
1バイトも違わないこと。KVのキーが7つちょうどで、元ファイルの `quant_format` などが
混入していないこと。`self-verify` が単独実行でも通ること。

#### 8-5-2. 現物照合のやり直し

8-3の照合を完全版に対して実行し直します。合格条件は、G-0〜G-Fが全件で合格すること
（G-0＝読めなかった54層が加わって1,440層になり、レポートの `markers_assumed` が0・
`markers_parsed` が1,440になること。形式の推定ではなく実際のマーカーを読んだ状態。
スクリプトの終了コードはG-0を含む全ゲートで決まる）です。あわせて、int8層831本の
マーカーがすべて `convrot: true` を明示していることを確認します（回転の有無はマーカーを
正とする契約のため、ここが崩れると回転なしで復元して無意味な出力になる。8-3の実測は
全int8層が回転済みであることを示している）。

#### 8-5-3. バックエンド実機ゲート（GPU使用）

GPUを使うため、実行前に必ず了承を得てください。実行中は他のGPU利用プロセス
（AviUtl2本体でのプレビュー生成など）を同時に動かさないこと。
...
- **マーカー本文の実読（8-5-2）で、int8側のマーカーが `convrot` を真で持っていることを
  必ず確認してください。** 実装はマーカーに `convrot` の記載が無ければ「回転なし」と
  解釈します（w4a8は形式上つねに回転ありなので影響しません）。8-3の照合は回転ありを
  仮定して高いcosが出ているため、実際の文面がそれと食い違っていないかの確認が要ります。
  想定外だった場合は、そこで止めて報告してください。
...
- `comfy_quant` の本文は完全版でしか読めません。8-4のマーカー長の説明（67/72の違いは
  JSONの書式の差）は、それまでは推測のままです。
