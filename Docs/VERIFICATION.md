...
### 9-8. 実機ゲート（GPU）— 実施済み（2026-09-17）

実行時刻: 10時29分〜10時34分。オーナー自身が00時47分から起動していたバックエンド
（v0.4.0、ポート18620、再起動なし）をそのまま使う了承を得て実施しました。
`config.yaml`の`model.backend`は`auto`で、ジョブのメタデータは
`backend: ltx-distilled`（＝お試し表示ではなく本物の推論）を記録しています。
GPUはNVIDIA GeForce RTX 4070 Ti SUPER（16GB）です。

操作はMCP（Model Context Protocol）経由で、Nz-Videomni側の`Docs/VERIFICATION_LOG.md`
§39.6にあるstdioクライアント方式（確立済みの手段）で行いました。使ったツールは
`backend_status`・`get_config`・`list_models`・`unload_pipeline`・`load_pipeline`
（`models={"transformer": …}`・`base_model="LTX23"`）・`submit_generate`・
`wait_for_job`・`job_status`・`get_job_video_path`です。クライアントスクリプトと
実行ログ（JSONL形式）はこのリポジトリの外にあります。

`list_models`で、Q6_K版が正しく見えることをまず確認しました。
`Sulphur-2-base-distil-Q6_K`がLTX 2.3のtransformer一覧に`exists: true`・
`source: "scan"`で並んでいました。

生成条件は2本ともまったく同一です。指示文 "A calm mountain lake at sunrise,
gentle ripples on the water, soft natural light, cinematic"・768×512・
49フレーム・24fps・seed 12345。その他はすべて既定（応答のechoで確認: 注意機構
`sdpa`、先読みブロックスワップ有効、`keep_resident`無効、融合GGUF逆量子化
カーネル有効、vae_modeは既定、8ステップ、guidance 1.0、LoRAなし、
ネガティブプロンプトなし）。

| 項目 | Q6_K | Q4_K_M |
|---|---|---|
| `load_pipeline`完了まで | 16.73秒 | 4.30秒 |
| ジョブのid | `9754fc09-317d-4c47-ae76-91b0e315bc3d` | `58a474a7-379e-485f-b27c-94e7e7940369` |
| バックエンド報告の生成時間 | 70.78秒 | 64.83秒（差 +5.95秒・+9.2%） |
| 出力ファイルサイズ | 177,357バイト | 183,253バイト |
| VRAMピーク（確保量） | 8,442MB | 8,442MB |
| VRAMピーク（予約量） | 8,972MB | 8,972MB |
| 装置全体のVRAMピーク（`nvidia-smi`・1秒ごと） | 9,629MiB | 9,568MiB（差 +61MiB） |
| ブロックスワップのCPU側マスター合計 | 14,533MB | 11,440MB |
| 一番大きいブロック | 302.8MB | 253.8MB |
| ピン留めステージングプール | 302.8MB×2 | 253.8MB×2 |

ジョブ全体のVRAMピーク（確保量・予約量）が2本で同じ値になっているのは、LTX 2.3
エンジンのジョブ全体ピークが除去（denoise）段階の外に立っているためです。
transformerの違いが直接反映されるのは、`nvidia-smi`で見た装置全体のピーク
（+61MiB）と、ブロックスワップのCPU側マスター合計（14,533MB対11,440MB）の
ほうです。

`load_pipeline`が完了した時点ではGGUF本体の量子化テンソルはまだ読み込まれて
おらず、実際の読み込みは最初のジョブの中で遅延して行われます（LTX 2.3・
LTX 2.5共通の設計。§8-5-3のBE-3と同じ挙動）。そのため`load_pipeline`の所要
時間（16.73秒／4.30秒）はパイプラインの立ち上げだけの時間で、GGUFの量子化
読み込みそのものは上表の「バックエンド報告の生成時間」に含まれています。
ワーカーログでの実測は、Q6_Kが約20秒（10時30分52秒〜10時31分12秒）、
Q4_K_Mが約17秒（10時32分51秒〜10時33分08秒）でした。

ワーカーログの内訳: 量子化読み込みは2本とも「2,812 float + 1,632 quantised
tensors」。先読みブロックスワップは2本とも10回のプリフェッチ・48ブロック発行・
同期ミス1回で、`server.log`・`ltx_worker.log`のどちらにも警告・エラーは
出ていません。

`ffprobe`で読んだ諸元は、ファイルサイズ以外2本とも同一でした: h264・768×512・
yuv420p・49フレーム・24fps・長さ2.0417秒・音声あり（aac）。

**絵としての見た目の評価はオーナーの担当です。** 見比べる対象のファイルは
次の2つです。

- Q6_K: `S:\OriginalApps\12_Nz-LTX23-AviUtl2\Nz-Videomni\outputs\9754fc09-317d-4c47-ae76-91b0e315bc3d\output.mp4`
- Q4_K_M: `S:\OriginalApps\12_Nz-LTX23-AviUtl2\Nz-Videomni\outputs\58a474a7-379e-485f-b27c-94e7e7940369\output.mp4`

後始末: バックエンドを、最初の状態（`Sulphur-2-base-distil-Q4_K_M`が読み込み
済み・`ready`）へ戻しました。デバイスメモリは`nvidia-smi`で開始前1,052MiB・
終了後1,041MiBとほぼ変わらず、Nz-Videomniの作業ツリーに差分はありません
（生成物は`outputs\`配下にでき、gitの管理の外にあります）。
